...
readonly YOCTO_SCRIPTS_PATH=${SCRIPT_POINT}/trucrux_scripts
...
echo "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
echo "~~  TRUCRUX recovery SD card creation script  ~~"
echo "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
...
function copy_images
{
	...
	if [ ${BOOT_ROM_SIZE} = 0 ]; then
		if [ ${MACHINE} = "imx8mq-trux-q01" ]; then
...
function copy_scripts
{
	...
	if [ ${HAS_DESKTOP_ICONS} = 1 ]; then
		...
		if [ -d ${P2_MOUNT_DIR}/usr/share/applications ]; then
			...
			if [[ ${YOCTO_RECOVERY_ROOTFS_BASE_IN_NAME} == trux-image-swupdate* ]]; then
